---
title: "Latitudinal temperature gradients estimated with Metropolis-Hastings"
...
gradient <- function(x, coeff, sd) {
  A = coeff[1]
  K = coeff[2]
  M = coeff[3]
  Q = coeff[4]
  nu = coeff[5]
  return(A + max(c(K-A,0))/((1+(nu*exp(Q*(x-M))))^(1/nu)) + rnorm(length(x),0,sd))
}
```
...
sample_data <- data.frame(
  x = sample_lat, 
  y = gradient(x = sample_lat, coeff = c(-2.2, 28, 39.1, 0.1, 1.2), sd = 2))
```
...
temperature <- gradient(x = latitude, coeff = c(-2.2, 28, 39.1, 0.1, 1.2), sd = 0)
...
```{r, warning = FALSE, echo = TRUE}
m <- run_MCMC(x = sample_data$x, y = sample_data$y, coeff_inits = c(10,15,20,1,1), sdy_init = 10, nIter = 20000)
```
To assess the model output, we produce traceplots of the parameters:
```{r, fig.width = 7, fig.height = 7, warning = FALSE, echo = FALSE}
par(mar = c(4,4,0.75,0.75), las = 1, mgp = c(2.25,0.75,0), cex = 1.35)
layout(matrix(1:6, nrow = 6, ncol = 1, byrow = TRUE))
plot(m$A,type = "l")
plot(m$K,type = "l")
plot(m$M,type = "l")
plot(m$Q,type = "l")
...
```
All the parameters seem to have converged reasonably well. We discard the first $10,000$ iterations as burn-in and use the mean of the parameters to plot the reconstructed gradient:
```{r, fig.width = 6, fig.height = 5, warning = FALSE, echo = FALSE}
burnin <- 1:10000
latseq <- seq(0,90,0.2)
par(mar = c(4,4.25,1.25,0.75), las = 1, mgp = c(2.25,0.75,0), cex = 1.35)
...
plot(latitude, temperature, type = "l", lwd = 1.5, ylim = c(-4,30), xlab = expression ("absolute latitude ("*degree*")"), yaxt = "n", ylab = expression("temperature ("*degree~"C)"), yaxs = "i", xaxs = "i",
     xlim = c(0,90))
points(sample_data$x, sample_data$y,  pch = 19, cex = 1.2, col = rgb(0,0,0,0.6), xpd = T)
points(latseq, gradient(x = latseq, coeff = apply(m[-burnin,1:5],2,mean), sd = 0), type = "l", lwd = 3, col = rgb(0.9,0,0,0.75))
axis(2,seq(-5,30,5),c(NA,0,NA,10,NA,20,NA,30))
