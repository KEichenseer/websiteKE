...
summary: To work well, the Metropolis algorithm needs proposals that are accepted reasonably often. Here, I show how draws from the posterior can be used to iteratively improve proposals, resulting in more efficient sampling.
...
We re-use the data from the [previous post](/post/a-metropolis-algorithm-in-r-part-1-implementation), and set the initial standard deviation of the proposals to $5$. This is probably too high, but we want to test whether the adaptation finds good values.
```{r, warning = FALSE, echo = FALSE}
### Taking samples
set.seed(10)
...
```
The traceplot below shows how the standard deviations of the proposals were adapted during the first $5000$ iterations. Despite a very bad initial guess for the $\sigma_{proposal}$ of $Q$, the adaptive algorithm managed to quickly find smaller, better values, resulting in reasonably good behaviour of the Markov chains of the parameters shown above. The effective sample size after adaptation is $`r round( mcmcse::multiESS(m$coeff[5001:nIter,]),0)`$, which is $`r round((mcmcse::multiESS(m$coeff[5001:nIter,])/2453 - 1)*100,0)`\%$ higher than the effective sample size obtained with the same number of iterations in the [previous version](/post/a-metropolis-algorithm-in-r-part-1-implementation) of this model, where we tried to guess good values for $\sigma_{proposal}$.
