...
run_MCMC <- function(x, y, coeff_inits, sdy_init, nIter, proposal_sd_init = rep(5,4), 
  ...
  coeff_diff <- array(NA_real_,dim = c(nAdapt,4)) # array to store the coeff. differences
  ...
  for (i in 2:nIter){
   ...
   ## 2. Metropolis-Hastings step to estimate the regression coefficients
    ...
    coeff_diff[i,] <- coefficients[i,]-coefficients[i-1,] # change of the coefficients
    if(i>=3) {
    weights = allWeights[(nAdapt-i+2):nAdapt-1] # select weights for current iteration
    sum_weights = sum(weights) 
    weighted_var_coeff <- apply(coeff_diff[2:i,], 2, # calculate weighted variance
