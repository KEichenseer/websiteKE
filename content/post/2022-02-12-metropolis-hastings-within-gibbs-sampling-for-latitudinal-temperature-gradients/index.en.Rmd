---
title: "Metropolis algorithm in R - Latitudinal temperature gradients"
author: "Kilian Eichenseer"
date: '2022-02-12'
slug: metropolis-algorithm-in-R-latitudinal-temperature-gradients
categories: []
tags: []
subtitle: ''
authors: []
...
summary: Reconstructing climate gradients from only a handful of data points is a challenge ideally suited for Bayesian modelling. Here, I build a Metropolis algorithm in R and use it to estimate a generalised logistic function.
---
...
To obtain posterior probabilities for the parameters of the gradient model, we use a Metropolis algorithm. The Metropolis algorithm builds a Markov chain which resembles the posterior distribution. This works by proposing random new values for the parameters, and calculating the posterior probability with the new parameter values. If the joint posterior probability obtained with the new values is higher than that of the current values, the new values will be recorded and added to the Markov chain. Otherwise, the new value will be accepted with a probabilty equal to the ratio of the two joint posterior probabilities.  
...
m <- run_MCMC(x = sample_data$x, y = sample_data$y, 
              coeff_inits = c(0,30,30,0.2), sdy_init = 4, nIter = nIter)
```
