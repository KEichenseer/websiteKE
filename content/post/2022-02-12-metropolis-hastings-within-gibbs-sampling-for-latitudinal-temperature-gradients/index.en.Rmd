...
```{r, warning = FALSE, echo = TRUE}
m <- run_MCMC(x = sample_data$x, y = sample_data$y, coeff_inits = c(10,15,25,0.5,0.5), sdy_init = 5, nIter = 20000)
```
To assess the model output, we produce traceplots of the parameters:
```{r, fig.width = 7, fig.height = 7, warning = FALSE, echo = FALSE, message = FALSE}
library(ggplot2)
library(dplyr)
library(reshape)
...
#ggplot(mm) + geom_histogram(aes(x = value))+
#      facet_grid(variable ~ ., switch = "x", scales = "free_x")+ 
...
```
The parameters have converged reasonably well, except for $\nu$. We discard the first $10,000$ iterations as burn-in and use the mean of the parameters to plot the reconstructed gradient:
```{r, fig.width = 5, fig.height = 4.25, warning = FALSE, echo = FALSE}
burnin <- 1:10000
latseq <- seq(0,90,0.2)
par(mar = c(4,4.25,1.25,0.75), las = 1, mgp = c(2.25,0.75,0), cex = 1.35)
